...
if (mySwitch.getReceivedValue() == 1200011) {                  // Comando para "SUBIR" o FLAP via RF.
           tempobotao = millis();
           delay(250);                                         // Tempo em milesegundos para enviar novamente o sinal RF.
                Subir();
}

else if (mySwitch.getReceivedValue() == 1200012) {             // Comando para "DESCER" o FLAP via RF.
           tempobotao = millis();
           delay(250);                                         // Tempo em milesegundos para enviar novamente o sinal RF.
               Descer();
}           
    
else if (mySwitch.getReceivedValue() == 1200013) {             // Comando para "PARAR" o FLAP via RF.
           tempobotao = millis();
           delay(250);                                         // Tempo em milesegundos para enviar novamente o sinal RF.
         PararGeral();
 }     
...
// ---- Loop Serial ---------------------------------------------------- Loop Serial ---------------------------------------------------- Loop Serial ----
 ...
 char letra = Serial.read();
   ...
   if (letra == 'u'){                       // Comando para "SUBIR" o FLAP via Serial.
   Subir();
   }
   
   else if (letra == 'i'){                  // Comando para "DESCER" o FLAP via Serial.
   Descer();
   }
   
   else if (letra == 'o'){                  // Comando para "PARAR" o FLAP via Serial.
   PararGeral();
  } 
 }
}
